...
class HomeScreenViewModel extends ViewModelBase {
  ...
  async savePersonInfoBlock() {
    try {
      ...
      personInfoBlockData.birthday =
        this.personInfoBlockData.birthday !== undefined &&
        this.personInfoBlockData.birthday !== ''
          ? this.toISODate(this.personInfoBlockData.birthday)
          : '';
      ...
      personInfoBlockData.gender = this.personInfoBlockData.gender;
      ...
      let has_to_check_rewards_again = true;
      ...
      if (canVerifyContact === true && contact_email !== '') {
        ...
        if (verifyResult.verification_needed === false) {
          if (
            ...
            has_to_check_rewards_again = false;
          } else {
            has_to_check_rewards_again = true;
      ...
      const saveGenderAndBirthday: any = await this.saveGenderAndBirthday(
        personInfoBlockData.gender,
        personInfoBlockData.birthday,
      );
      if (saveGenderAndBirthday.success === true) {
        await this.refreshUserData();
        this.savingPersonInfoData = false;
        await this.initValues();
        if (has_to_check_rewards_again) {
          if (
            saveGenderAndBirthday.data.rewards &&
            saveGenderAndBirthday.data.rewards.basic_data === true &&
            oldRewards.basic_data === false
          ) {
            const coin = '+' + this.static_rewards.basic_data_reward;
            verifyResultMessage = GetMessage(
