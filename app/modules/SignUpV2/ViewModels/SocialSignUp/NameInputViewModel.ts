...
class NameInputViewModel extends ViewModelBase {
  ...
  async onPressContinue() {
    standardFunctions.add_firebase_event_log('scl_signup', 'btn_name_clicked');
    this.loading = true;
    this.updateView();
  ...
  componentDidMount() {
    // @ts-ignore
    const data = global.navigationData['data'];
    this.nickname = '';
    this.email = data.email;
    this.firstname = data.firstname;
    this.lastname = data.lastname;
    this.profile_image_url = data.profile_image_url;
    this.facebook_access_token = data.facebook_access_token;
